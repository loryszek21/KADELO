...
export default function NewUser({ className, props, setTab, router}: SignInProps): JSX.Element{
    ...
    const login = async (event: SubmitEvent) => {
        ...
        const res = await fetch('http://localhost:5000/user/insertUser', {
            ...
            body: JSON.stringify({email: email.current, password: password.current, name: name.current})
        })
        .then(res => {
    ...
    return (
        <>
            <form className={styles.login_form} onSubmit={(e: any) => login(e)}>
                <P size='l' className={styles.title}>Registration</P>
